...
- ✅ **Обход ограничений YouTube через браузерные куки**
...
- **Обход возрастных ограничений** - использование браузерных кук
...
├── extract_cookies.py   # Инструмент для извлечения кук (локальное использование)
...
YouTube блокирует доступ к контенту с возрастными ограничениями без авторизации. Этот раздел описывает как настроить куки для обхода ограничений.
...
A: Нет, сервис работает только с публично доступными видео. Приватные видео недоступны. Однако можно загружать контент с возрастными ограничениями используя браузерные куки.
...
**Q: Ошибка "Sign in to confirm your age"**
A: YouTube требует авторизации для контента с возрастными ограничениями. Настройте браузерные куки (см. раздел "Настройка кук").

**Q: Куки не работают в Docker**
A: Куки нужно извлекать локально и передавать в конфигурацию. Docker контейнер не имеет доступа к браузерам.
